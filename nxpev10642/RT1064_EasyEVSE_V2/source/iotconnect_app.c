...
#if (USE_SE_DEVICE_CERTIFICATE == 1)
UCHAR se_device_cert[SE05X_CLIENT_CERTIFICATE_SIZE];
size_t se_device_certLength = sizeof(se_device_cert);
#endif
...
bool app_startup(NX_IP *ip_ptr, NX_PACKET_POOL *pool_ptr, NX_DNS *dns_ptr) {
   ...
   config->auth.data.x509.auth_interface.get_cert(
    ...
    extern const unsigned char dummy_device_private_key_ptr[];

#if (USE_SE_DEVICE_CERTIFICATE == 1)
    #ifndef ENABLE_DPS_SAMPLE
	 se05x_GetCertificate(SSS_CERTIFICATE_INDEX_CLIENT, se_device_cert, &se_device_certLength );
    #endif
#endif

	struct sw_der_driver_parameters dp = {0};
	dp.cert = (uint8_t *) (se_device_cert);
	dp.cert_size = se_device_certLength;
	dp.key = (uint8_t *) (dummy_device_private_key_ptr);
	dp.key_size = dummy_device_private_key_len;
