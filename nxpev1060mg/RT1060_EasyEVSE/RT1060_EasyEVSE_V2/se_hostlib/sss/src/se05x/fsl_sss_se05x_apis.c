...
#if (__GNUC__ && !AX_EMBEDDED) || (USE_RTOS) || (USE_THREADX_RTOS)
#define USE_LOCK 1
#else
#define USE_LOCK 0
#endif
...
sss_status_t sss_se05x_tunnel_context_init(sss_se05x_tunnel_context_t *context, sss_se05x_session_t *session)
{
    ...
    sss_status_t retval    = kStatus_SSS_Success;
#if USE_THREADX_RTOS
    if(tx_mutex_create(&context->channelLock, "channelLock_mutex", TX_NO_INHERIT) != TX_SUCCESS){
    	LOG_E("\n channelLock tx_mutex_create failed");
        return kStatus_SSS_Fail;
    }
#elif USE_RTOS
...
{
#if USE_THREADX_RTOS
    	tx_mutex_delete(&context->channelLock);
#elif USE_RTOS
