...
#if defined(PHDRIVER_IMXRT106XRC663_BOARD)
#define PHDRIVER_IMXRT_1060
#endif
...
#ifdef   DEBUG
    #ifdef defined(PHDRIVER_KINETIS_K82) || defined(PHDRIVER_IMXRT_1060)
...
#define CHECK_STATUS(x)                                      \
    if ((x) != PH_ERR_SUCCESS)                               \
{                                                            \
PRINTF("Line: %d   Error - (0x%04X) has occurred : 0xCCEE CC-Component ID, EE-Error code. Refer-ph_Status.h\n", __LINE__, (x));    \
}
