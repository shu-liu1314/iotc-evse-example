...
UINT len;
    ...
    while (az_result_succeeded(az_json_reader_next_token(json_reader)))
    {
        if (json_reader -> token.kind == AZ_JSON_TOKEN_PROPERTY_NAME)
        {
            if (az_result_failed(az_json_token_get_string(&(json_reader -> token), (CHAR *)scratch_buf, (INT)scratch_buf_len, (int32_t *)&len)))
            {
...
UINT index;
    ...
    if (az_result_failed(az_json_reader_init(&json_reader, payload, NX_NULL)) ||
    ...
    while (az_result_succeeded(az_json_reader_next_token(&json_reader)))
    {
        if (json_reader.token.kind == AZ_JSON_TOKEN_PROPERTY_NAME)
        {
            if (az_result_failed(az_json_token_get_string(&(json_reader.token), (CHAR *)scratch_buf,
...
az_span component_name = az_span_create(component_name_ptr, (INT)component_name_len);
    ...
    if (az_result_succeeded(az_json_writer_init(&json_builder, buff_span, NULL)) &&
        ...
        (component_name_ptr == NX_NULL ||
         (az_result_succeeded(az_json_writer_append_property_name(&json_builder, component_name)) &&
          az_result_succeeded(az_json_writer_append_begin_object(&json_builder)) &&
        ...
        (component_name_ptr == NX_NULL || az_result_succeeded(az_json_writer_append_end_object(&json_builder))) &&
...
az_span description = az_span_create(description_ptr, (INT)description_len);
    ...
    if (!(az_result_succeeded(az_json_writer_append_property_name(&json_builder, reported_property_name)) &&
          az_result_succeeded(az_json_writer_append_begin_object(&json_builder)) &&
          az_result_succeeded(az_json_writer_append_property_name(&json_builder, reported_value_property_name)) &&
          (append_value(&json_builder, context) == NX_AZURE_IOT_SUCCESS) &&
          az_result_succeeded(az_json_writer_append_property_name(&json_builder, reported_status_property_name)) &&
          az_result_succeeded(az_json_writer_append_int32(&json_builder, result)) &&
          az_result_succeeded(az_json_writer_append_property_name(&json_builder, reported_description_property_name)) &&
          az_result_succeeded(az_json_writer_append_string(&json_builder, description)) &&
          az_result_succeeded(az_json_writer_append_property_name(&json_builder, reported_version_property_name)) &&
          az_result_succeeded(az_json_writer_append_int32(&json_builder, (INT)ack_version)) &&
          az_result_succeeded(az_json_writer_append_end_object(&json_builder)) &&
          az_result_succeeded(az_json_writer_append_end_object(&json_builder))))
    {
        PRINTF("Failed build reported property with status message\r\n");
