...
#ifdef PHDRIVER_IMXRT_1060
static void phApp_IMXRT1060_Init(void);
#endif
...
extern char CardUID[20];
extern char CardTek[2];  /* A, B, F, V*/
extern char CardType[4]; /* 1 , 2, 3, P2P */
extern uint8_t SizeUID;
...
#elif defined(PHDRIVER_IMXRT_1060)
    phApp_IMXRT1060_Init();
#else
...
void phApp_PrintTech(uint8_t TechType)
{
    ...
    case PHAC_DISCLOOP_POS_BIT_MASK_A:
        PRINTF ("\tResolving Type A... \n");
        break;

    case PHAC_DISCLOOP_POS_BIT_MASK_B:
        PRINTF ("\tResolving Type B... \n");
        break;

    case PHAC_DISCLOOP_POS_BIT_MASK_F212:
        PRINTF ("\tResolving Type F with baud rate 212... \n");
        break;

    case PHAC_DISCLOOP_POS_BIT_MASK_F424:
        PRINTF ("\tResolving Type F with baud rate 424... \n");
        break;

    case PHAC_DISCLOOP_POS_BIT_MASK_V:
        PRINTF ("\tResolving Type V... \n");
        break;
...
/**
* This function will convert the card UID
* into a hexadecimal string respresentation
* ideal for printing to screen or cloud
* \param   *pBuff   Buffer Reference
* \param   num      data size to be print
*/
void phApp_Update_UID(uint8_t *pBuff, uint8_t num)
{
    uint32_t    i;
    char InHex[3];
    ...
    for(i = 0; i < num; i++)
    {
        sprintf(InHex, "%02X", pBuff[i]);
        strncat(CardUID,InHex, 2);
    }
}
...
void phApp_PrintTagInfo(phacDiscLoop_Sw_DataParams_t *pDataParams, uint16_t wNumberOfTags, uint16_t wTagsDetected)
{
    ...
    if (PHAC_DISCLOOP_CHECK_ANDMASK(wTagsDetected, PHAC_DISCLOOP_POS_BIT_MASK_A))
    {
        if(pDataParams->sTypeATargetInfo.bT1TFlag)
        {
            strcpy(CardTek, "A");
            strcpy(CardType, "1");
            SizeUID = (pDataParams->sTypeATargetInfo.aTypeA_I3P3[0].bUidSize);
            phApp_Update_UID(pDataParams->sTypeATargetInfo.aTypeA_I3P3[0].aUid, SizeUID);
        }
        else
        {
            strcpy(CardTek, "A");
            ...
            {
                SizeUID = (pDataParams->sTypeATargetInfo.aTypeA_I3P3[bIndex].bUidSize);
                phApp_Update_UID(pDataParams->sTypeATargetInfo.aTypeA_I3P3[bIndex].aUid, SizeUID);
    ...
    if (PHAC_DISCLOOP_CHECK_ANDMASK(wTagsDetected, PHAC_DISCLOOP_POS_BIT_MASK_B))
    {
        ...
        for (bIndex = 0; bIndex < wNumberOfTags; bIndex++)
        {
            ...
            SizeUID = 4;
            phApp_Update_UID( pDataParams->sTypeBTargetInfo.aTypeB_I3P3[bIndex].aPupi, SizeUID);
        }
        strcpy(CardType, "3");
    }
#endif
        ...
        PHAC_DISCLOOP_CHECK_ANDMASK(wTagsDetected, PHAC_DISCLOOP_POS_BIT_MASK_F424))
    {
        ...
        for (bIndex = 0; bIndex < wNumberOfTags; bIndex++)
        {
            ...
            SizeUID = PHAC_DISCLOOP_FELICA_IDM_LENGTH;
            phApp_Update_UID(pDataParams->sTypeFTargetInfo.aTypeFTag[bIndex].aIDmPMm, SizeUID);
    ...
    if (PHAC_DISCLOOP_CHECK_ANDMASK(wTagsDetected, PHAC_DISCLOOP_POS_BIT_MASK_V))
    {
        ...
        {
            SizeUID = 8;
            phApp_Update_UID(pDataParams->sTypeVTargetInfo.aTypeV[bIndex].aUid, SizeUID);
    ...
    {
        PRINTF("\tTechnology  : ISO 18000p3m3 / EPC Gen2");
...
void PrintErrorInfo(phStatus_t wStatus)
{
    ...
    case PH_COMP_PAL_ISO14443P3A:
        PRINTF("\t PH_COMP_PAL_ISO14443P3A");
        break;
    case PH_COMP_PAL_ISO14443P3B:
        PRINTF("\t PH_COMP_PAL_ISO14443P3B");
        break;
    case PH_COMP_PAL_ISO14443P4A:
        PRINTF("\t PH_COMP_PAL_ISO14443P4A");
        break;
    case PH_COMP_PAL_ISO14443P4:
        PRINTF("\t PH_COMP_PAL_ISO14443P4");
        break;
    case PH_COMP_PAL_FELICA:
        PRINTF("\t PH_COMP_PAL_FELICA");
        break;
    case PH_COMP_PAL_EPCUID:
        PRINTF("\t PH_COMP_PAL_EPCUID");
        break;
    case PH_COMP_PAL_SLI15693:
        PRINTF("\t PH_COMP_PAL_SLI15693");
        break;
    case PH_COMP_PAL_I18000P3M3:
        PRINTF("\t PH_COMP_PAL_I18000P3M3");
        break;
    case PH_COMP_PAL_I18092MPI:
        PRINTF("\t PH_COMP_PAL_I18092MPI");
        break;
    case PH_COMP_PAL_I18092MT:
        PRINTF("\t PH_COMP_PAL_I18092MT");
        break;
    case PH_COMP_PAL_GENERALTARGET:
        PRINTF("\t PH_COMP_PAL_GENERALTARGET");
        break;
    case PH_COMP_PAL_I14443P4MC:
        PRINTF("\t PH_COMP_PAL_I14443P4MC");
        break;
    case PH_COMP_AC_DISCLOOP:
        PRINTF("\t PH_COMP_AC_DISCLOOP");
        break;
    case PH_COMP_OSAL:
        PRINTF("\t PH_COMP_OSAL");
        break;
    default:
        PRINTF("\t 0x%x",(wStatus & PH_COMPID_MASK));
        break;
    }
    ...
    case PH_ERR_SUCCESS_INCOMPLETE_BYTE:
        PRINTF("\t PH_ERR_SUCCESS_INCOMPLETE_BYTE");
        break;
    case PH_ERR_IO_TIMEOUT:
        PRINTF("\t PH_ERR_IO_TIMEOUT");
        break;
    case PH_ERR_INTEGRITY_ERROR:
        PRINTF("\t PH_ERR_INTEGRITY_ERROR");
        break;
    case PH_ERR_COLLISION_ERROR:
        PRINTF("\t PH_ERR_COLLISION_ERROR");
        break;
    case PH_ERR_BUFFER_OVERFLOW:
        PRINTF("\t PH_ERR_BUFFER_OVERFLOW");
        break;
    case PH_ERR_FRAMING_ERROR:
        PRINTF("\t PH_ERR_FRAMING_ERROR");
        break;
    case PH_ERR_PROTOCOL_ERROR:
        PRINTF("\t PH_ERR_PROTOCOL_ERROR");
        break;
    case PH_ERR_RF_ERROR:
        PRINTF("\t PH_ERR_RF_ERROR");
        break;
    case PH_ERR_EXT_RF_ERROR:
        PRINTF("\t PH_ERR_EXT_RF_ERROR");
        break;
    case PH_ERR_NOISE_ERROR:
        PRINTF("\t PH_ERR_NOISE_ERROR");
        break;
    case PH_ERR_ABORTED:
        PRINTF("\t PH_ERR_ABORTED");
        break;
    case PH_ERR_INTERNAL_ERROR:
        PRINTF("\t PH_ERR_INTERNAL_ERROR");
        break;
    case PH_ERR_INVALID_DATA_PARAMS:
        PRINTF("\t PH_ERR_INVALID_DATA_PARAMS");
        break;
    case PH_ERR_INVALID_PARAMETER:
        PRINTF("\t PH_ERR_INVALID_PARAMETER");
        break;
    case PH_ERR_PARAMETER_OVERFLOW:
        PRINTF("\t PH_ERR_PARAMETER_OVERFLOW");
        break;
    case PH_ERR_UNSUPPORTED_PARAMETER:
        PRINTF("\t PH_ERR_UNSUPPORTED_PARAMETER");
        break;
    case PH_ERR_OSAL_ERROR:
        PRINTF("\t PH_ERR_OSAL_ERROR");
        break;
    case PHAC_DISCLOOP_LPCD_NO_TECH_DETECTED:
        PRINTF("\t PHAC_DISCLOOP_LPCD_NO_TECH_DETECTED");
        break;
    case PHAC_DISCLOOP_COLLISION_PENDING:
        PRINTF("\t PHAC_DISCLOOP_COLLISION_PENDING");
        break;
    default:
        PRINTF("\t 0x%x",(wStatus & PH_ERR_MASK));
        break;
    }
}
